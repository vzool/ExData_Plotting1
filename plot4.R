...
plot4 <- function(){
  ...
  plot(d$Global_active_power, type="l", xaxt="n")
  axis(1, at=c(1,nrow(d)/2,nrow(d)), labels=c("Thu", "Fri", "Sat"))
  
  plot(d$Voltage, ylab="Voltage", type="l", xaxt="n", xlab="datetime")
  axis(1, at=c(1,nrow(d)/2,nrow(d)), labels=c("Thu", "Fri", "Sat"))
  ...
  plot(d$Global_reactive_power, type="l", xaxt="n", xlab="index")
  axis(1, at=c(1,nrow(d)/2,nrow(d)), labels=c("Thu", "Fri", "Sat"))
  
  dev.off()
  message(paste0("Image created in ", getwd(), "/plot4.png"))
}